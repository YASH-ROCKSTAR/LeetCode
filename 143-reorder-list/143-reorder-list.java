...
//Main process it that in this question is that we take beginning elemet then last element 
//as follows then second node then second last as follows
class Solution {
    public void reorderList(ListNode head) {
        ...
        ListNode first=head;
        //so process is that we have to take last node we get fromsecond 4-5 i want 5 from second first node 
        //so I will take revbese of that second node that mid.next
        // and marking mid.enxt ==null as break the list
        ListNode second=reverse(mid.next);
        mid.next=null;
        //continue till second is null as second will small in case of odd so that why we marked till second is null
        while(second!=null){
            // first we store first next in temp because when we will link 1-5 then 1 will lost connectivity with 2 so storing 2 in temp
    ...
    // taking the reverse of mid.next as when we take second list then it will come in 
